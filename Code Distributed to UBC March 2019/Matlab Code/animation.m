function [] = animation(axes,Bat,dstr,figure,az,el)
...
%
    ...
    %Sturm-Pelvis
    strlmb = [Bat(1,str),Bat(1,lmb);Bat(2,str),Bat(2,lmb);Bat(3,str),Bat(3,lmb)];
    
    D = extend(dstr,2);
    
    plot3(axes,strlmb(1,:)-D(1,:),strlmb(2,:)-D(2,:),strlmb(3,:)-D(3,:),'g','LineWidth',5)
    hold on
    ...
    if Left == 0
        % basic configuration
        legR = [Bat(1,lmb),Bat(1,ankR);Bat(2,lmb),Bat(2,ankR);Bat(3,lmb),Bat(3,ankR)];
        size = 2;
        % if the right knee is tracked then it's added to the configuration
        if isfinite(kneeR) == 1
            legR = [legR(:,1) [Bat(1,kneeR);Bat(2,kneeR);Bat(3,kneeR)] legR(:,size)];
            size = size + 1;
        end
        % if the right hip is tracked then it's added to the configuration
        if isfinite(hipR) == 1
            legR = [legR(:,1) [Bat(1,hipR);Bat(2,hipR);Bat(3,hipR)] legR(:,2:size)];
            size = size + 1;
        end
        D = extend(dstr,size);
     
        plot3(axes,legR(1,:)-D(1,:),legR(2,:)-D(2,:),legR(3,:)-D(3,:),'r','LineWidth',5)
    ...
    %Leg L
        % basic configuration
        legL = [Bat(1,lmb),Bat(1,ankL);Bat(2,lmb),Bat(2,ankL);Bat(3,lmb),Bat(3,ankL)];
        size = 2;
        % if the left knee is tracked then it's added to the configuration
        if isfinite(kneeL) == 1
            legL = [legL(:,1) [Bat(1,kneeL);Bat(2,kneeL);Bat(3,kneeL)] legL(:,size)];
            size = size + 1;
        end
        % if the left hip is tracked then it's added to the configuration
        if isfinite(hipL) == 1
            legL = [legL(:,1) [Bat(1,hipL);Bat(2,hipL);Bat(3,kneeL)] legL(:,2:size)];
            size = size + 1;
        end
        D = extend(dstr,size);
     
        plot3(axes,legL(1,:)-D(1,:),legL(2,:)-D(2,:),legL(3,:)-D(3,:),'r','LineWidth',5)
    
        
    end
    
    % Mark the "head" (in front of the sternum)
    plot3(axes,Bat(1,str)-dstr(1,1),Bat(2,str)-dstr(1,2),Bat(3,str)-dstr(1,3), 'ro','MarkerSize',15,'MarkerFaceColor',[.5,.5,.5]) 
    ...
    if Left == 0
        ...
        armR = [Bat(1,shdR),Bat(1,elbR),Bat(1,wstR),Bat(1,t3R); ...
            ...
            Bat(3,shdR),Bat(3,elbR),Bat(3,wstR),Bat(3,t3R)];
        ...
        if isfinite(ip3R) == 1
            armR = [armR(:,1:3) [Bat(1,ip3R); Bat(2,ip3R); Bat(3,ip3R)] armR(:,size)];
            size = size + 1;
        end
        % if mcp3R is tracked then it's added to the configuration
        if isfinite(mcp3R) == 1
            armR = [armR(:,1:3) [Bat(1,mcp3R); Bat(2,mcp3R); Bat(3,mcp3R)] armR(:,4:size)];
            size = size + 1;
        end
        % if Sn06R is tracked then it's added to the configuration
        if isfinite(Sn06R) == 1
            armR = [armR(:,1:3) [Bat(1,Sn06R); Bat(2,Sn06R); Bat(3,Sn06R)] armR(:,4:size)];
            size = size + 1;
        end
        % if Sn03R is tracked then it's added to the configuration
        if isfinite(Sn03R) == 1
            armR = [armR(:,1:2) [Bat(1,Sn03R); Bat(2,Sn03R); Bat(3,Sn03R)] armR(:,3:size)];
            size = size + 1;
        end
        % if Sn02R is tracked then it's added to the configuration
        if isfinite(Sn02R) == 1
            armR = [armR(:,1:1) [Bat(1,Sn02R); Bat(2,Sn02R); Bat(3,Sn02R)] armR(:,2:size)];
            size = size + 1;
        end

        D = extend(dstr, size);

        plot3(axes,armR(1,:)-D(1,:),armR(2,:)-D(2,:),armR(3,:)-D(3,:),'r','LineWidth',5)
    end
    ...
    if Right == 0
        ...
        armL = [Bat(1,shdL),Bat(1,elbL),Bat(1,wstL),Bat(1,t3L); ...
            ...
            Bat(3,shdL),Bat(3,elbL),Bat(3,wstL),Bat(3,t3L)];
        ...
        if isfinite(ip3L) == 1
            armL = [armL(:,1:4) [Bat(1,ip3L); Bat(2,ip3L); Bat(3,ip3L)] armL(:,size)];
            size = size + 1;
        end
        % if mcp3L is tracked then it's added to the configuration
        if isfinite(mcp3L) == 1
            armL = [armL(:,1:3) [Bat(1,mcp3L); Bat(2,mcp3L); Bat(3,mcp3L)] armL(:,4:size)];
            size = size + 1;
        end
        % if Sn06L is tracked then it's added to the configuration
        if isfinite(Sn06L) == 1
            armL = [armL(:,1:3) [Bat(1,Sn06L); Bat(2,Sn06L); Bat(3,Sn06L)] armL(:,4:size)];
            size = size + 1;
        end
        % if Sn03L is tracked then it's added to the configuration
        if isfinite(Sn03L) == 1
            armL = [armL(:,1:2) [Bat(1,Sn03L); Bat(2,Sn03L); Bat(3,Sn03L)] armL(:,3:size)];
            size = size + 1;
        end
        % if Sn02L is tracked then it's added to the configuration
        if isfinite(Sn02L) == 1
            armL = [armL(:,1:1) [Bat(1,Sn02L); Bat(2,Sn02L); Bat(3,Sn02L)] armL(:,2:size)];
            size = size + 1;
        end

        D = extend(dstr, size);

        plot3(axes,armL(1,:)-D(1,:),armL(2,:)-D(2,:),armL(3,:)-D(3,:),'r','LineWidth',5)
    end
    ...
    if Left ==0
        ...
        fng4R = [Bat(1,wstR),Bat(1,t4R);Bat(2,wstR),Bat(2,t4R);...
            Bat(3,wstR),Bat(3,t4R)];
        size = 2;
        
        
        % if ip4R is tracked then it's added to the configuration
        if isfinite(ip4R) == 1
            fng4R = [fng4R(:,1) [Bat(1,ip4R); Bat(2,ip4R); Bat(3,ip4R)] fng4R(:,size)];
            size = size + 1;
        end
        % if mcp4R is tracked then it's added to the configuration
        if isfinite(mcp4R) == 1
            fng4R = [fng4R(:,1) [Bat(1,mcp4R); Bat(2,mcp4R); Bat(3,mcp4R)] fng4R(:,2:size)];
            size = size + 1;
        end
        % if Sn05R is tracked then it's added to the configuration
        if isfinite(Sn05R) == 1
            fng4R = [fng4R(:,1) [Bat(1,Sn05R); Bat(2,Sn05R); Bat(3,Sn05R)] fng4R(:,2:size)];
            size = size + 1;
        end
        D = extend(dstr,size);
        
        plot3(axes,fng4R(1,:)-D(1,:),fng4R(2,:)-D(2,:),fng4R(3,:)-D(3,:), 'r','LineWidth',5)
    ...
    %4th Finger L
        ...
        fng4L = [Bat(1,wstL),Bat(1,t4L);Bat(2,wstL),Bat(2,t4L);...
            Bat(3,wstL),Bat(3,t4L)];
        size = 2;
        
        
        % if ip4L is tracked then it's added to the configuration
        if isfinite(ip4L) == 1
            fng4L = [fng4L(:,1) [Bat(1,ip4L); Bat(2,ip4L); Bat(3,ip4L)] fng4L(:,size)];
            size = size + 1;
        end
        % if mcp4L is tracked then it's added to the configuration
        if isfinite(mcp4L) == 1
            fng4L = [fng4L(:,1) [Bat(1,mcp4L); Bat(2,mcp4L); Bat(3,mcp4L)] fng4L(:,2:size)];
            size = size + 1;
        end
        % if Sn05L is tracked then it's added to the configuration
        if isfinite(Sn05L) == 1
            fng4L = [fng4L(:,1) [Bat(1,Sn05L); Bat(2,Sn05L); Bat(3,Sn05L)] fng4L(:,2:size)];
            size = size + 1;
        end
        D = extend(dstr,size);
        
        plot3(axes,fng4L(1,:)-D(1,:),fng4L(2,:)-D(2,:),fng4L(3,:)-D(3,:), 'r','LineWidth',5)
    ...
    if Left == 0
        ...
        fng5R = [Bat(1,wstR),Bat(1,t5R);Bat(2,wstR),Bat(2,t5R);...
            Bat(3,wstR),Bat(3,t5R)];
        size = 2;
        
        % if ip5R is tracked then it's added to the configuration
        if isfinite(ip5R) == 1
            fng5R = [fng5R(:,1) [Bat(1,ip5R); Bat(2,ip5R); Bat(3,ip5R)] fng5R(:,size)];
            size = size + 1;
        end
        % if mcp5R is tracked then it's added to the configuration
        if isfinite(mcp5R) == 1
            fng5R = [fng5R(:,1) [Bat(1,mcp5R); Bat(2,mcp5R); Bat(3,mcp5R)] fng5R(:,2:size)];
            size = size + 1;
        end
        % if Sn04R is tracked then it's added to the configuration
        if isfinite(Sn04R) == 1
            fng5R = [fng5R(:,1) [Bat(1,Sn04R); Bat(2,Sn04R); Bat(3,Sn04R)] fng5R(:,2:size)];
            size = size + 1;
        end
        D = extend(dstr,size);
        
        plot3(axes,fng5R(1,:)-D(1,:),fng5R(2,:)-D(2,:),fng5R(3,:)-D(3,:), 'r','LineWidth',5)
    ...
    %5th Finger L
        ...
        fng5L = [Bat(1,wstL),Bat(1,t5L);Bat(2,wstL),Bat(2,t5L);...
            Bat(3,wstL),Bat(3,t5L)];
        size = 2;
        
        % if ip5L is tracked then it's added to the configuration
        if isfinite(ip5L) == 1
            fng5L = [fng5L(:,1) [Bat(1,ip5L); Bat(2,ip5L); Bat(3,ip5L)] fng5L(:,size)];
            size = size + 1;
        end
        % if mcp5L is tracked then it's added to the configuration
        if isfinite(mcp5L) == 1
            fng5L = [fng5L(:,1) [Bat(1,mcp5L); Bat(2,mcp5L); Bat(3,mcp5L)] fng5L(:,2:size)];
            size = size + 1;
        end
        % if Sn04L is tracked then it's added to the configuration
        if isfinite(Sn04L) == 1
            fng5L = [fng5L(:,1) [Bat(1,Sn04L); Bat(2,Sn04L); Bat(3,Sn04L)] fng5L(:,2:size)];
            size = size + 1;
        end
        D = extend(dstr,size);
        
        plot3(axes,fng5L(1,:)-D(1,:),fng5L(2,:)-D(2,:),fng5L(3,:)-D(3,:), 'r','LineWidth',5)
    ...
    if Left == 0
        ...
        teR = [Bat(1,ankR),Bat(1,t5R),Bat(1,t4R),Bat(1,t3R);Bat(2,ankR),Bat(2,t5R),Bat(2,t4R),Bat(2,t3R); ...
            Bat(3,ankR),Bat(3,t5R),Bat(3,t4R),Bat(3,t3R)];
        size = 4;
        % if Te09R is tracked then it's added to the configuration
        if isfinite(Te09R) == 1
            teR = [teR(:,1:3) [Bat(1,Te09R); Bat(2,Te09R); Bat(3,Te09R)] teR(:,4)];
            size = size + 1;
        end
        % if Te08R is tracked then it's added to the configuration
        if isfinite(Te08R) == 1
            teR = [teR(:,1:3) [Bat(1,Te08R); Bat(2,Te08R); Bat(3,Te08R)] teR(:,4:size)];
            size = size + 1;
        end
        % if Te07R is tracked then it's added to the configuration
        if isfinite(Te07R) == 1
            teR = [teR(:,1:3) [Bat(1,Te07R); Bat(2,Te07R); Bat(3,Te07R)] teR(:,4:size)];
            size = size + 1;
        end
        % if Te06R is tracked then it's added to the configuration
        if isfinite(Te06R) == 1
            teR = [teR(:,1:2) [Bat(1,Te06R); Bat(2,Te06R); Bat(3,Te06R)] teR(:,3:size)];
            size = size + 1;
        end
        % if Te05R is tracked then it's added to the configuration
        if isfinite(Te05R) == 1
            teR = [teR(:,1:2) [Bat(1,Te05R); Bat(2,Te05R); Bat(3,Te05R)] teR(:,3:size)];
            size = size + 1;
        end
        % if Te04R is tracked then it's added to the configuration
        if isfinite(Te04R) == 1
            teR = [teR(:,1:2) [Bat(1,Te04R); Bat(2,Te04R); Bat(3,Te04R)] teR(:,3:size)];
            size = size + 1;
        end
        % if Te03R is tracked then it's added to the configuration
        if isfinite(Te03R) == 1
            teR = [teR(:,1) [Bat(1,Te03R); Bat(2,Te03R); Bat(3,Te03R)] teR(:,2:size)];
            size = size + 1;
        end
        % if Te02R is tracked then it's added to the configuration
        if isfinite(Te02R) == 1
            teR = [teR(:,1) [Bat(1,Te02R); Bat(2,Te02R); Bat(3,Te02R)] teR(:,2:size)];
            size = size + 1;
        end
        % if Te01R is tracked then it's added to the configuration
        if isfinite(Te01R) == 1
            teR = [teR(:,1) [Bat(1,Te01R); Bat(2,Te01R); Bat(3,Te01R)] teR(:,2:size)];
            size = size + 1;
        end
        
        D = extend(dstr,size);
        
        plot3(axes,teR(1,:)-D(1,:),teR(2,:)-D(2,:),teR(3,:)-D(3,:), 'g','LineWidth',3)
    ...
    if Right == 0
        ...
        teL = [Bat(1,ankL),Bat(1,t5L),Bat(1,t4L),Bat(1,t3L);Bat(2,ankL),Bat(2,t5L),Bat(2,t4L),Bat(2,t3L); ...
            Bat(3,ankL),Bat(3,t5L),Bat(3,t4L),Bat(3,t3L)];
        size = 4;
        % if Te09L is tracked then it's added to the configuration
        if isfinite(Te09L) == 1
            teL = [teL(:,1:3) [Bat(1,Te09L); Bat(2,Te09L); Bat(3,Te09L)] teL(:,4)];
            size = size + 1;
        end
        % if Te08L is tracked then it's added to the configuration
        if isfinite(Te08L) == 1
            teL = [teL(:,1:3) [Bat(1,Te08L); Bat(2,Te08L); Bat(3,Te08L)] teL(:,4:size)];
            size = size + 1;
        end
        % if Te07L is tracked then it's added to the configuration
        if isfinite(Te07L) == 1
            teL = [teL(:,1:3) [Bat(1,Te07L); Bat(2,Te07L); Bat(3,Te07L)] teL(:,4:size)];
            size = size + 1;
        end
        % if Te06L is tracked then it's added to the configuration
        if isfinite(Te06L) == 1
            teL = [teL(:,1:2) [Bat(1,Te06L); Bat(2,Te06L); Bat(3,Te06L)] teL(:,3:size)];
            size = size + 1;
        end
        % if Te05L is tracked then it's added to the configuration
        if isfinite(Te05L) == 1
            teL = [teL(:,1:2) [Bat(1,Te05L); Bat(2,Te05L); Bat(3,Te05L)] teL(:,3:size)];
            size = size + 1;
        end
        % if Te04L is tracked then it's added to the configuration
        if isfinite(Te04L) == 1
            teL = [teL(:,1:2) [Bat(1,Te04L); Bat(2,Te04L); Bat(3,Te04L)] teL(:,3:size)];
            size = size + 1;
        end
        % if Te03L is tracked then it's added to the configuration
        if isfinite(Te03L) == 1
            teL = [teL(:,1) [Bat(1,Te03L); Bat(2,Te03L); Bat(3,Te03L)] teL(:,2:size)];
            size = size + 1;
        end
        % if Te02L is tracked then it's added to the configuration
        if isfinite(Te02L) == 1
            teL = [teL(:,1) [Bat(1,Te02L); Bat(2,Te02L); Bat(3,Te02L)] teL(:,2:size)];
            size = size + 1;
        end
        % if Te01L is tracked then it's added to the configuration
        if isfinite(Te01L) == 1
            teL = [teL(:,1) [Bat(1,Te01L); Bat(2,Te01L); Bat(3,Te01L)] teL(:,2:size)];
            size = size + 1;
        end
        
        D = extend(dstr,size);
        
        plot3(axes,teL(1,:)-D(1,:),teL(2,:)-D(2,:),teL(3,:)-D(3,:), 'g','LineWidth',3)
    ...
    sj = 0;
        ...
        leR = [Bat(1,shdR),Bat(1,wstR),Bat(1,t3R);Bat(2,shdR),Bat(2,wstR),Bat(2,t3R); ...
            Bat(3,shdR),Bat(3,wstR),Bat(3,t3R)];
        size = 3;
        
        % if Sj21R is tracked then it's added to the configuration
        if isfinite(Sj21R) == 1
            leR = [leR(:,1:2) [Bat(1,Sj21R); Bat(2,Sj21R); Bat(3,Sj21R)] leR(:,size)];
            size = size + 1;  
        end
        % if ip3R is tracked then it's added to the configuration
        if isfinite(ip3R) == 1
            leR = [leR(:,1:2) [Bat(1,ip3R); Bat(2,ip3R); Bat(3,ip3R)] leR(:,3:size)];
            size = size + 1;
        end
        % if mcp3R is tracked then it's added to the configuration
        if isfinite(mcp3R) == 1
            leR = [leR(:,1:2) [Bat(1,mcp3R); Bat(2,mcp3R); Bat(3,mcp3R)] leR(:,3:size)];
            size = size + 1;
        end
        % if Sj20R is tracked then it replaces mcp3R in the configuration
        if isfinite(Sj20R) == 1
            leR(:,3) = [Bat(1,Sj20R); Bat(2,Sj20R); Bat(3,Sj20R)];            
        end
        % if Sn06R is tracked then it's added to the configuration
        if isfinite(Sn06R) == 1
            leR = [leR(:,1:2) [Bat(1,Sn06R); Bat(2,Sn06R); Bat(3,Sn06R)] leR(:,3:size)];
            size = size + 1;
        end
        % if Sj19R is tracked then it replaces wstR in the configuration
        if isfinite(Sj19R) == 1
            leR(:,2) = [Bat(1,Sj19R); Bat(2,Sj19R); Bat(3,Sj19R)];
        ...
        if isfinite(Sj18R) == 1
            ...
            % configuration
            if sj == 0
                leR(:,2) = [Bat(1,Sj18R); Bat(2,Sj18R); Bat(3,Sj18R)];
            % if sj19 is tracked then it is added to the configuration
            elseif sj == 1
                leR = [leR(:,1) [Bat(1,Sj18R); Bat(2,Sj18R); Bat(3,Sj18R)] leR(:,2:size)];
                size = size + 1;
            end
        end
        % if Le03R is tracked then it's added to the configuration
        if isfinite(Le03R) == 1
            leR = [leR(:,1) [Bat(1,Le03R); Bat(2,Le03R); Bat(3,Le03R)] leR(:,2:size)];
            size = size + 1;
        end
        % if Le02R is tracked then it's added to the configuration
        if isfinite(Le02R) == 1
            leR = [leR(:,1) [Bat(1,Le02R); Bat(2,Le02R); Bat(3,Le02R)] leR(:,2:size)];
            size = size + 1;
        end
        % if Le01R is tracked then it's added to the configuration
        if isfinite(Le01R) == 1
            leR = [leR(:,1) [Bat(1,Le01R); Bat(2,Le01R); Bat(3,Le01R)] leR(:,2:size)];
            size = size + 1;
        end
        
        D = extend(dstr,size);
        
        plot3(axes,leR(1,:)-D(1,:),leR(2,:)-D(2,:),leR(3,:)-D(3,:), 'y','LineWidth',3)
    ...
    if Right == 0
        ...
        leL = [Bat(1,shdL),Bat(1,wstL),Bat(1,t3L);Bat(2,shdL),Bat(2,wstL),Bat(2,t3L); ...
            Bat(3,shdL),Bat(3,wstL),Bat(3,t3L)];
        size = 3;
        % if Sj21L is tracked then it's added to the configuration
        if isfinite(Sj21L) == 1
            leL = [leL(:,1:2) [Bat(1,Sj21L); Bat(2,Sj21L); Bat(3,Sj21L)] leL(:,size)];
            size = size + 1;  
        end
        % if ip3L is tracked then it's added to the configuration
        if isfinite(ip3L) == 1
            leL = [leL(:,1:3) [Bat(1,ip3L); Bat(2,ip3L); Bat(3,ip3L)] leL(:,3:size)];
            size = size + 1;
        end
        % if mcp3L is tracked then it's added to the configuration
        if isfinite(mcp3L) == 1
            leL = [leL(:,1:2) [Bat(1,mcp3L); Bat(2,mcp3L); Bat(3,mcp3L)] leL(:,3:size)];
            size = size + 1;
        end
        % if Sn06L is tracked then it's added to the configuration
        if isfinite(Sn06L) == 1
            leL = [leL(:,1:2) [Bat(1,Sn06L); Bat(2,Sn06L); Bat(3,Sn06L)] leL(:,3:size)];
            size = size + 1;
        end
        % if Sj19L is tracked then it replaces wstL in the configuration
        if isfinite(Sj19L) == 1
            leL(:,2) = [Bat(1,Sj19L); Bat(2,Sj19L); Bat(3,Sj19L)];
        ...
        if isfinite(Sj18L) == 1
            ...
            % configuration
            if sj == 0
                leL(:,2) = [Bat(1,Sj18L); Bat(2,Sj18L); Bat(3,Sj18L)];
            % if sj19 is tracked then it is added to the configuration
            elseif sj == 1
                leL = [leL(:,1) [Bat(1,Sj18L); Bat(2,Sj18L); Bat(3,Sj18L)] leL(:,2:size)];
                size = size + 1;
            end
        end
        % if Le03L is tracked then it's added to the configuration
        if isfinite(Le03L) == 1
            leL = [leL(:,1) [Bat(1,Le03L); Bat(2,Le03L); Bat(3,Le03L)] leL(:,2:size)];
            size = size + 1;
        end
        % if Le02L is tracked then it's added to the configuration
        if isfinite(Le02L) == 1
            leL = [leL(:,1) [Bat(1,Le02L); Bat(2,Le02L); Bat(3,Le02L)] leL(:,2:size)];
            size = size + 1;
        end
        % if Le01L is tracked then it's added to the configuration
        if isfinite(Le01L) == 1
            leL = [leL(:,1) [Bat(1,Le01L); Bat(2,Le01L); Bat(3,Le01L)] leL(:,2:size)];
            size = size + 1;
        end
        
        D = extend(dstr,size);
        
        plot3(axes,leL(1,:)-D(1,:),leL(2,:)-D(2,:),leL(3,:)-D(3,:), 'y','LineWidth',3)
    ...
    for i=1:+1:W/3
        V(i,:) = [Bat(1,i)-dstr(1,1),Bat(2,i)-dstr(1,2),Bat(3,i)-dstr(1,3)];
